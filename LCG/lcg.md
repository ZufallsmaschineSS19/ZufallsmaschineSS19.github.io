...
<p>We divide the binary sequence into segments of four bits each, <span class="math display">\[ 0010, 0001, 0101, 0101, 0010, 1100, 1100, 0101, 1110, 0010, 1011, \dots \]</span> We can turn each segment into a four bit number between <span class="math inline">\(0\)</span> to <span class="math inline">\(15\)</span>. There are exactly <span class="math inline">\(16\)</span> different numbers. We now count the occurrence of each of these numbers, denoted <span class="math inline">\(s_{i}\)</span> with <span class="math inline">\(i \in [0, 15]\)</span>. With this in mind we calculate the sum <span class="math display">\[X_{s} = \sum_{i=0}^{i=15} s_{i}^{2} - 5000\]</span></p>
